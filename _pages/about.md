---
permalink: /
title: "Francesco De Vanna"
excerpt: "Tenure-Track Assistant Professor in Aerospace Propulsion - University of Padova & CISAS"
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

I am a **Tenure-Track Assistant Professor in Aerospace Propulsion** at the Department of Industrial Engineering of the [University of Padova](https://www.unipd.it/en/), and a member of **CISAS** — the "Giuseppe Colombo" Center of Studies and Activities for Space.

## Research

My work is in **computational fluid dynamics**, with a focus on:

- compressible and turbulent **wall-bounded flows** and **shock/boundary-layer interaction**;
- **film cooling** for gas-turbine and aero-propulsion systems;
- **large-eddy simulation** and high-fidelity numerical methods;
- **high-performance computing** — I am the lead developer of [**URANOS**](https://github.com/uranos-gpu/uranos-gpu), a GPU-accelerated solver for the compressible Navier–Stokes equations built to run efficiently on modern supercomputers.

## Software

Alongside my research I develop **open-source scientific software** and work with modern HPC and Python stacks, with an interest in applying **machine learning** to fluid mechanics and engineering problems.

Browse my [publications](/publications/) and [CV](/cv/) from the menu above, or reach me at [francesco.devanna@unipd.it](mailto:francesco.devanna@unipd.it).
